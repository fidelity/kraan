...
function install_prometheus {
  ...
  echo "You can access grafana at http:/127.0.0.1:3000"
  echo "grafana user: `kubectl get secret -n "${ns}" prometheus-grafana -o json | jq -r '.data["admin-user"]' | base64 -d`"
  echo "grafana password: `kubectl get secret -n "${ns}" prometheus-grafana -o json | jq -r '.data["admin-password"]' | base64 -d`"
}
